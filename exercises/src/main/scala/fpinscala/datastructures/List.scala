...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  def tail[A](l: List[A]): List[A] = List.drop(l, 1)
  ...
  def foldRight2[A,B](l: List[A], z: B)(f: (A, B) => B): B = foldLeft(reverse(l), z)((z, x) => f(x, z))

  def append2[A](a1: List[A], a2: List[A]): List[A] = foldRight(a1, a2)((x, z) => setHead(z, x))
  ...
  def toStringList(l: List[Double]): List[String] = l match {
    case Nil => Nil
  ...
  def add2Lists(l1: List[Int], l2: List[Int]): List[Int] = (l1, l2) match {
    case (Nil, _)                   => Nil
    case (_, Nil)                   => Nil
